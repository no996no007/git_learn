...
def set_world_linear_by_dst_point(xyzo):
    ...
    if(abs(max_speed)>3):
        x_speed=3*x_speed/abs(max_speed)
        y_speed=3*y_speed/abs(max_speed)
        z_speed=3*z_speed/abs(max_speed)
    else:
    ...
    if(abs(uav_yaw)<3.1):
        odom.twist.twist.angular.z = 0.8*uav_yaw/abs(uav_yaw) # 暂时使它一直朝向yaw=3.14的位置

    if (max(np.abs([x_speed,y_speed,z_speed])/Kp)<0.03): # 最大距离差小于0.05判断为到达目的
        return True
    else:
        return False
